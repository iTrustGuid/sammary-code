class Promise {
    ...
    then (onFufilled, onRejected) {
        ...
        function resolvePromise (promise2, x, resolve, reject) {
            ...
            if (x != null && (Promise.isFunction(x) || Promise.isObject(x))) { //可能是promise
                try {
                    let then = x.then; // 如果是对象 我就试着取一下then方法 如果有then，认为它是promise
                    if (Promise.isFunction(then)) { // then是函数，是promise
                        then.call(x, y => {
                            // 成功和失败只能调用一个
                            if (called) return;
                            called = true;
        ...
        promise2 = new Promise((resolve, reject) => {
            if (this.status === 'resolved') {
                setTimeout(() => {
                    try {
                        //x是上一个promise返回值，可能是一个普通值，也可能是一个promise；
                        //x也可能是别人的promise，我们可以写一个方法，统一处理
                        let x = onFufilled(this.value);
                        //下一次then的实例promise2，这次返回值x，promise2的成功方法，promise2的失败方法
